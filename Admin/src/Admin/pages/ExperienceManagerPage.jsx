  ...
  const columns = [
    {
      ...
      render: (val, item) => (
        <div className="space-y-0.5 max-w-md">
          <div className="font-bold text-slate-900 dark:text-white">{val || item.designation}</div>
          <div className="text-[11px] text-slate-500 dark:text-zinc-400">{item.organization || item.institute}</div>
        </div>
      ),
    },
    {
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      <div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
          <Briefcase className="w-5 h-5 text-blue-600 dark:text-zinc-300" /> Teaching & Administrative Experience
        </h1>
