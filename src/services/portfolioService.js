...
// API Bridge helper to seamlessly save/load between Admin and Public website in local dev
const apiBridge = {
...
const createTableService = (tableName, defaultSort = 'display_order') => {
  return {
    async getAll(includeDrafts = true) {
      // 1. Try local data API first for immediate dev sync if running dev server
  ...
  async getDashboardMetrics() {
    try {
      const [
        ...
        this.projects.getAll(true),
        this.skills.getAll(true),
        this.publications.getAll(true),
        this.patents.getAll(true),
        this.experience.getAll(true),
        this.achievements.getAll(true),
        this.books.getAll(true),
        this.talksAndEvents.getAll(true),
        this.guidance.getAll(true),
      ]);
      ...
      return {
        ...
        totalPublications: (pubsRes.data || []).length,
        totalPatents: (patentsRes.data || []).length,
        totalExperience: (expRes.data || []).length,
        totalAchievements: (achRes.data || []).length,
        totalBooks: (booksRes.data || []).length,
        totalTalks: (talksRes.data || []).length,
        totalScholars: (guidanceRes.data || []).length,
      };
    } catch (err) {
      return {
        totalProjects: 11,
